...
def test_jwe():
    ...
    data = jwe.serialize_compact(bytes('533', encoding='utf-8'), key)
    print(data)
    decrypted_data = jwe.deserialize_compact(data, key)
...
def test_decrypt_jwe():
    ...
    data = jwe.deserialize_compact(bytes(jweString, encoding='utf-8'), encryptionKey)
