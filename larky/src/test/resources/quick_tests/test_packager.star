load("@stdlib//pprint", pprint="pprint")
 
load("@vendor//iso8583", iso8583="iso8583")
load("@vendor//iso8583/specs", spec="spec", default_ascii="default_ascii")
...
def MyTestCase_test_file():
    # `with` and file reading are not supported
    with open("iso.dat", "rb") as binary_file:
        # with open("0200.dat", "rb") as binary_file:
        data = binary_file.read()[2:]
    print(data)
        decoded, encoded = iso8583.decode(data, my_spec)
    pprint.pprint(decoded)
...
#    _suite.addTest(unittest.FunctionTestCase(MyTestCase_test_file))
    return _suite
