...
# load("@vendor//Crypto/Hash", SHA1="SHA1", HMAC="HMAC", SHA256="SHA256", MD5="MD5", SHA224="SHA224", SHA384="SHA384", SHA512="SHA512")
# load("@vendor//Crypto/Protocol/KDF", _bcrypt_decode="_bcrypt_decode")
load("@vendor//Crypto/Protocol/KDF", PBKDF1="PBKDF1", PBKDF2="PBKDF2", _S2V="S2V", HKDF="HKDF", scrypt="scrypt", bcrypt="bcrypt", bcrypt_check="bcrypt_check")
load("@vendor//Crypto/Util/py3compat", b="b", bchr="bchr")
...
def TestVector():
    self = larky.mutablestruct(__name__='TestVector')
    return self
...
def PBKDF1_Tests_test1():
    v = PBKDF1_testData[0]
    res = PBKDF1(v[0], t2b(v[1]), v[2], v[3], "SHA1")
    asserts.assert_that(res).is_equal_to(t2b(v[4]))
...
def PBKDF2_Tests_test1():
    # Test only for HMAC-SHA1 as PRF

    def PBKDF2_Tests_PBKDF2_Tests_prf_SHA1(p,s):
        return HMAC.new(p,s,SHA1).digest()

    def PBKDF2_Tests_PBKDF2_Tests_prf_SHA256(p,s):
        return HMAC.new(p,s,SHA256).digest()

    for i in range(len(PBKDF2_testData)):
        v = PBKDF2_testData[i]
        password = v[0]
        salt = t2b(v[1])
        out_len = v[2]
        ...
        if hash_mod == SHA1:
            ...
            asserts.assert_that(res).is_equal_to(expected)

            res = PBKDF2(password, salt, out_len, iters, PBKDF2_Tests_PBKDF2_Tests_prf_SHA1)
            asserts.assert_that(res).is_equal_to(expected)
        else:
            res = PBKDF2(password, salt, out_len, iters, PBKDF2_Tests_PBKDF2_Tests_prf_SHA256)
            asserts.assert_that(res).is_equal_to(expected)

def PBKDF2_Tests_test2():
    # Verify that prf and hmac_hash_module are mutual exclusive
    def PBKDF2_Tests_PBKDF2_Tests_prf_SHA1(p,s):
        return HMAC.new(p,s,SHA1).digest()

    asserts.assert_fails(lambda: PBKDF2(b("xxx"), b("yyy"), 16, 100,
                      prf=PBKDF2_Tests_PBKDF2_Tests_prf_SHA1, hmac_hash_module=SHA1), ".*?ValueError")

def PBKDF2_Tests_test3():
...
#             s2v.update(t2b(s))
#         result = s2v.derive()
#         asserts.assert_that(result).is_equal_to(t2b(tv[2]))
#
# def S2V_Tests_test2():
...
#             s2v.update(b("XX"))
#         asserts.assert_fails(lambda: s2v.update(b("YY")), ".*?TypeError")
...
#     data = new_test_vectors
#
# def scrypt_Tests_test2():
#
#     for tv in data:
#         try:
...
#                 return e
#         asserts.assert_that(output).is_equal_to(tv.output)
#
# def scrypt_Tests_test3():
...
#     asserts.assert_that((ref[:4], ref[4:8], ref[8:])).is_equal_to((key1, key2, key3))
#
# def bcrypt_Tests_test_negative_cases():
#     asserts.assert_fails(lambda: bcrypt(b"1" * 73, 10), ".*?ValueError")
#     asserts.assert_fails(lambda: bcrypt(b"1" * 10, 3), ".*?ValueError")
#     asserts.assert_fails(lambda: bcrypt(b"1" * 10, 32), ".*?ValueError")
#     asserts.assert_fails(lambda: bcrypt(b"1" * 10, 4, salt=b""), ".*?ValueError")
#     asserts.assert_fails(lambda: bcrypt(b"1" * 10, 4, salt=b"1"), ".*?ValueError")
#     asserts.assert_fails(lambda: bcrypt(b"1" * 10, 4, salt=b"1" * 17), ".*?ValueError")
#     asserts.assert_fails(lambda: bcrypt(b"1\x00" * 10, 4), ".*?ValueError")
#
# def bcrypt_Tests_test_bytearray_mismatch():
#     ref = bcrypt("pwd", 4)
...
#
# def bcrypt_Tests_test_empty_password():
...
#         asserts.assert_that(x).is_equal_to(result)
#         bcrypt_check(password, result)
#
# def bcrypt_Tests_test_random_password_and_salt_short_pw():
#     # password, cost, salt, bcrypt hash
#     tvs = [
...
#         bcrypt_check(password, result)
#
# def bcrypt_Tests_test_random_password_and_salt_long_pw():
#     # password, cost, salt, bcrypt hash
#     tvs = [
...
#         bcrypt_check(password, result)
#
# def bcrypt_Tests_test_same_password_and_random_salt():
#     # password, cost, salt, bcrypt hash
#     tvs = [
...
#         bcrypt_check(password, result)
#
# def bcrypt_Tests_test_same_password_and_salt_increasing_cost_factor():
#     # password, cost, salt, bcrypt hash
#     tvs = [
...
#         bcrypt_check(password, result)
#
# def bcrypt_Tests_test_long_passwords():
#     # password, cost, salt, bcrypt hash
#     tvs = [
...
#         bcrypt_check(password, result)
#
# def bcrypt_Tests_test_increasing_password_length():
#     # password, cost, salt, bcrypt hash
#     tvs = [
...
#         bcrypt_check(password, result)
#
# def bcrypt_Tests_test_non_ascii_characters():
#     # password, cost, salt, bcrypt hash
#     tvs = [
...
#         bcrypt_check(password, result)
#
# def bcrypt_Tests_test_special_case_salt():
#     # password, cost, salt, bcrypt hash
#     tvs = [
...
def _testsuite():
    ...
    _suite.addTest(unittest.FunctionTestCase(PBKDF1_Tests_test1))
    _suite.addTest(unittest.FunctionTestCase(PBKDF2_Tests_test1))
    _suite.addTest(unittest.FunctionTestCase(PBKDF2_Tests_test2))
    _suite.addTest(unittest.FunctionTestCase(PBKDF2_Tests_test3))
    _suite.addTest(unittest.FunctionTestCase(PBKDF2_Tests_test4))
    # _suite.addTest(unittest.FunctionTestCase(S2V_Tests_test1))
    # _suite.addTest(unittest.FunctionTestCase(S2V_Tests_test2))
    # _suite.addTest(unittest.FunctionTestCase(HKDF_Tests_test1))
    # _suite.addTest(unittest.FunctionTestCase(HKDF_Tests_test2))
    # _suite.addTest(unittest.FunctionTestCase(scrypt_Tests_test2))
    # _suite.addTest(unittest.FunctionTestCase(scrypt_Tests_test3))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_negative_cases))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_bytearray_mismatch))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_empty_password))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_random_password_and_salt_short_pw))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_random_password_and_salt_long_pw))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_same_password_and_random_salt))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_same_password_and_salt_increasing_cost_factor))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_long_passwords))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_increasing_password_length))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_non_ascii_characters))
    # _suite.addTest(unittest.FunctionTestCase(bcrypt_Tests_test_special_case_salt))
    return _suite
