...
load("@vendor//Crypto/Protocol/KDF", PBKDF1="PBKDF1", PBKDF2="PBKDF2", _S2V="S2V", HKDF="HKDF", scrypt="scrypt", bcrypt="bcrypt", _bcrypt_decode="bcrypt_decode", bcrypt_check="bcrypt_check")
...
def bcrypt_Tests_test_bytearray_mismatch():
    ...
    # wrong = b"x" + ref[1:]
    wrong = bytearray("x", encoding='utf-8') + ref[1:]
    asserts.assert_fails(lambda: bcrypt_check("pwd", wrong), ".*?ValueError")
...
def bcrypt_Tests_test_empty_password():
    ...
    for (idx, (password, cost, salt64, result)) in enumerate(tvs):
        ...
        # asserts.assert_that(x).is_equal_to(result) # TODO: Fix
        asserts.assert_that(bytearray(x)).is_equal_to(bytearray(result))
