...
load("@vendor//Crypto/Cipher/PKCS1_OAEP", PKCS="PKCS1_OAEP")
...
load("@vendor//Crypto/Hash/SHA512", SHA384="SHA512")
...
def t2b(t):
    ...
    clean = rws(t)
    if len(clean) % 2 == 1:
        return Error("ValueError: Even number of characters expected")
...
def randGen_testDecrypt1():
    ...
    for test in _testData:
        ...
        asserts.assert_that(pt).is_equal_to(t2b(test[1]))

def randGen_testDecrypt2():
    # Simplest possible negative tests
    key1024 = PKCS1_OAEP_Tests_setUp()
    for ct_size in (127, 128, 129):
        cipher = PKCS.new(key1024)
        asserts.assert_fails(lambda: cipher.decrypt(bchr(0x00)*ct_size), ".*?ValueError")

def randGen_testEncryptDecrypt1():
...
def randGen_testEncryptDecrypt2():
    ...
    for hashmod in (MD5,
                    # MD2,  # TODO(mahmoudimus): add
                    SHA1,
                    # RIPEMD160,   # TODO(mahmoudimus): add
                    SHA256):
        # Verify that encrypt() asks for as many random bytes
        # as the hash output size
        asked[0] = 0
        pt = _rng(40)
        cipher = PKCS.new(key1024, hashmod, randfunc=localRng)
        ct = cipher.encrypt(pt)
        asserts.assert_that(cipher.decrypt(ct)).is_equal_to(pt)
        asserts.assert_that(asked[0]).is_equal_to(hashmod.digest_size)


def randGen_testEncryptDecrypt3():
    # Verify that OAEP supports labels
    key1024 = PKCS1_OAEP_Tests_setUp()
    pt = _rng(35)
    xlabel = _rng(22)
    cipher = PKCS.new(key1024, label=xlabel)
    ct = cipher.encrypt(pt)
    asserts.assert_that(cipher.decrypt(ct)).is_equal_to(pt)


def randGen_testEncryptDecrypt4():
    # Verify that encrypt() uses the custom MGF
    mgfcalls = [0]
    # Helper function to monitor what's requested from MGF

    def newMGF(seed, maskLen):
        mgfcalls[0] = mgfcalls[0] + 1
        return b'\x00' * maskLen

    pt = _rng(32)
    key1024 = PKCS1_OAEP_Tests_setUp()
    cipher = PKCS.new(key1024, mgfunc=newMGF)
    ct = cipher.encrypt(pt)
    asserts.assert_that(mgfcalls[0]).is_equal_to(2)
    asserts.assert_that(cipher.decrypt(ct)).is_equal_to(pt)
...
#
# def TestVectorsWycheproof___init__(wycheproof_warnings, skip_slow_tests):
#     unittest.TestCase.__init__(self)
#     _wycheproof_warnings = wycheproof_warnings
#     _skip_slow_tests = skip_slow_tests
#     _id = "None"
#
# def TestVectorsWycheproof_load_tests(filename):
#
#     def TestVectorsWycheproof_TestVectorsWycheproof_filter_rsa(group):
#         return RSA.import_key(group['privateKeyPem'])
#
#     def TestVectorsWycheproof_TestVectorsWycheproof_filter_sha(group):
#         if group['sha'] == "SHA-1":
#             return SHA1
...
#             return SHA512
#         else:
#             return Error("ValueError: " + "Unknown sha " + group['sha'])
#
#     def TestVectorsWycheproof_TestVectorsWycheproof_filter_mgf(group):
...
#         else:
#             return Error("ValueError: " + "Unknown mgf/sha " + group['mgfSha'])
#
#     def TestVectorsWycheproof_TestVectorsWycheproof_filter_algo(group):
...
#     return result
#
# def TestVectorsWycheproof_setUp():
#     tv = []
#     tv.extend(load_tests("rsa_oaep_2048_sha1_mgf1sha1_test.json"))
#     tv.extend(load_tests("rsa_oaep_2048_sha224_mgf1sha1_test.json"))
#     tv.extend(load_tests("rsa_oaep_2048_sha224_mgf1sha224_test.json"))
#     tv.extend(load_tests("rsa_oaep_2048_sha256_mgf1sha1_test.json"))
#     tv.extend(load_tests("rsa_oaep_2048_sha256_mgf1sha256_test.json"))
#     tv.extend(load_tests("rsa_oaep_2048_sha384_mgf1sha1_test.json"))
#     tv.extend(load_tests("rsa_oaep_2048_sha384_mgf1sha384_test.json"))
#     tv.extend(load_tests("rsa_oaep_2048_sha512_mgf1sha1_test.json"))
#     tv.extend(load_tests("rsa_oaep_2048_sha512_mgf1sha512_test.json"))
#     if not _skip_slow_tests:
#         tv.extend(load_tests("rsa_oaep_3072_sha256_mgf1sha1_test.json"))
#         tv.extend(load_tests("rsa_oaep_3072_sha256_mgf1sha256_test.json"))
#         tv.extend(load_tests("rsa_oaep_3072_sha512_mgf1sha1_test.json"))
#         tv.extend(load_tests("rsa_oaep_3072_sha512_mgf1sha512_test.json"))
#         tv.extend(load_tests("rsa_oaep_4096_sha256_mgf1sha1_test.json"))
#         tv.extend(load_tests("rsa_oaep_4096_sha256_mgf1sha256_test.json"))
#         tv.extend(load_tests("rsa_oaep_4096_sha512_mgf1sha1_test.json"))
#         tv.extend(load_tests("rsa_oaep_4096_sha512_mgf1sha512_test.json"))
#         tv.extend(load_tests("rsa_oaep_4096_sha512_mgf1sha512_test.json"))
#         tv.extend(load_tests("rsa_oaep_misc_test.json"))
#
# def TestVectorsWycheproof_shortDescription():
#     return _id
#
# def TestVectorsWycheproof_warn(tv):
#     if tv.warning and _wycheproof_warnings:
#         load("@stdlib//warnings", warnings="warnings")
#         warnings.warn("Wycheproof warning: %s (%s)" % (_id, tv.comment))
#
# def TestVectorsWycheproof_test_decrypt(tv):
#     _id = "Wycheproof Decrypt %s Test #%s" % (tv.algo, tv.id)
#
...
#
#
# def TestVectorsWycheproof_get_tests(config={}):
...
def _testsuite():
    ...
    _suite.addTest(unittest.FunctionTestCase(randGen_testDecrypt1))
    _suite.addTest(unittest.FunctionTestCase(randGen_testDecrypt2))
    _suite.addTest(unittest.FunctionTestCase(randGen_testEncryptDecrypt1))
    _suite.addTest(unittest.FunctionTestCase(randGen_testEncryptDecrypt2))
    _suite.addTest(unittest.FunctionTestCase(randGen_testEncryptDecrypt3))
    _suite.addTest(unittest.FunctionTestCase(randGen_testEncryptDecrypt4))
