...
import com.verygood.security.larky.nativelib.LarkyAssertions;
...
public final class LarkyScriptTester {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (String name : testdata.list()) {
      ...
      for (String chunk : Splitter.on("\n---\n").split(content)) {
        ...
        Starlark.addMethods(predeclared, new LarkyAssertions()); // e.g. assert_eq
        ...
        try (Mutability mu = Mutability.create("test")) {
          ...
          thread.setThreadLocal(LarkyAssertions.Reporter.class, new LarkyAssertions.Reporter() {
            @Override
            public void reportError(StarlarkThread thread, String message) {
              LarkyAssertions.Reporter.super.reportError(thread,message);
              ok = false;
            }
          });
