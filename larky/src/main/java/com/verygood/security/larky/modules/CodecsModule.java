...
public class CodecsModule implements StarlarkValue {
  ...
  public StarlarkList<StarlarkInt> encode(String strToEncode, String encoding, String errors) throws EvalException {
    TextUtil textUtil = new TextUtil(strToEncode);
    return StarlarkList.immutableCopyOf(Bytes.asList(textUtil.copyBytes()).stream().map(Byte::toUnsignedInt).map(StarlarkInt::of).collect(Collectors.toList()));
  }
