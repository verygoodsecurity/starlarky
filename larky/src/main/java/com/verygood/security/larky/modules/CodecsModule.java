...
public class CodecsModule implements StarlarkValue {
  ...
  @StarlarkMethod(
    ...
    public String decode(StarlarkList<StarlarkInt> bytesToDecode, String encoding, String errors) throws EvalException {
    List<Number> collected = bytesToDecode.stream().map(StarlarkInt::toNumber).collect(Collectors.toList());
    byte[] barr = Bytes.toArray(collected);
    try {
      return TextUtil.decode(barr);
