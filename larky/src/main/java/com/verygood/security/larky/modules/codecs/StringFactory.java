...
public final class StringFactory {
    ...
    public static String newStringFromBytes(byte[] data, int offset, int byteCount, String charsetName) throws UnsupportedEncodingException {
        return newStringFromBytes(data, offset, byteCount, Charset.forName(charsetName));
    }
    public static String newStringFromBytes(byte[] data, String charsetName) throws UnsupportedEncodingException {
        return newStringFromBytes(data, 0, data.length, Charset.forName(charsetName));
    }
    ...
    public static String newStringFromBytes(byte[] data, int offset, int byteCount, Charset charset) {
        if ((offset | byteCount) < 0 || byteCount > data.length - offset) {
          throw new StringIndexOutOfBoundsException(String.format("%d, %d, %d", data.length, offset, byteCount));
        }
        char[] value;
        int length;
...
outer:
    ...
    public static String newStringFromChars(char[] data, int offset, int charCount) {
        if ((offset | charCount) < 0 || charCount > data.length - offset) {
          throw new StringIndexOutOfBoundsException(String.format("%d, %d, %d", data.length, offset, charCount));
        }

      List<Character> c = Chars.asList(data).stream().skip(offset).limit(charCount).collect(Collectors.toList());
   ...
   // TODO: Implement this method natively.
    public static String newStringFromCodePoints(int[] codePoints, int offset, int count) {
        ...
        if ((offset | count) < 0 || count > codePoints.length - offset) {
            throw new StringIndexOutOfBoundsException(String.format("%d, %d, %d", codePoints.length, offset, count));
        }
