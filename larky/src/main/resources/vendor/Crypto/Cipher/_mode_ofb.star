...
def OfbMode(block_cipher, iv):
    ...
    def __init__(block_cipher, iv):
        ...
        if result:
            fail(" ValueError(\"Error %d while instantiating the OFB mode\"\n                             % result)")
    ...
    def encrypt(plaintext, output=None):
        ...
        if result:
            fail(" ValueError(\"Error %d while encrypting in OFB mode\" % result)")
    ...
    def decrypt(ciphertext, output=None):
        ...
        if result:
            fail(" ValueError(\"Error %d while decrypting in OFB mode\" % result)")
...
def _create_ofb_cipher(factory, **kwargs):
    ...
    if kwargs:
        fail(" TypeError(\"Unknown parameters for OFB: %s\" % str(kwargs))")
