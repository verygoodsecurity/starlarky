...
load("@vendor//Crypto/Util/py3compat", byte_string="byte_string")
...
def _create_base_cipher(dict_parameters):
    ...
    if len(key) not in key_size:
        return Error("ValueError: Incorrect ARC2 key length (%d bytes)" % len(key)).unwrap()

    if not ((40 <= effective_keylen) and (effective_keylen <= 1024)):
        return Error("ValueError: 'effective_key_len' must be at least 40 " +
                     "and no larger than 1024 (not %d)" % effective_keylen).unwrap()
    #
    ...
    #                          cipher.address_of())
    # if result:
    #     return Error("ValueError: " + "Error %X while instantiating the ARC2 cipher"
    #                      % result)
    #
    # return SmartPointer(cipher.get(), stop_operation)
    return Error("ValueError: Error while instantiating the ARC2 cipher").unwrap()
...
def new(key, mode, *args, **kwargs):
    ...
    return Cipher._create_cipher(ARC2, key, mode, *args, **kwargs)
