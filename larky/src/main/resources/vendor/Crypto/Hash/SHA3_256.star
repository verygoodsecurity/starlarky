...
def SHA3_256_Hash(data=None, update_after_digest=False):
    """
    ...
    def __init__(data, update_after_digest):
        """
        Error %d while instantiating SHA-3/256

        """
        self_ = {'_update_after_digest': update_after_digest, '_digest_done': False}
        self_['_state'] = _JCrypto.Hash.SHA3_256()
        if data:
...
def new(*args, **kwargs):
    ...
    return SHA3_256_Hash().new(data, update_after_digest)


SHA3_256 = larky.struct(
