...
load("@vendor//cryptography/exceptions", InvalidSignature="InvalidSignature")
load("@vendor//cryptography/hazmat/backends", default_backend="default_backend", openssl="openssl")
load("@vendor//cryptography/hazmat/primitives", hashes="hashes")
load("@vendor//cryptography/hazmat/primitives/asymmetric", rsa="rsa", padding="padding", dsa="dsa")
load("@vendor//cryptography/hazmat/primitives/ciphers", Cipher="Cipher", algorithms="algorithms", modes="modes")
load("@vendor//cryptography/hazmat/primitives/interfaces", RSAPrivateKey="RSAPrivateKey", RSAPublicKey="RSAPublicKey", DSAPublicKey="DSAPublicKey", DSAPrivateKey="DSAPrivateKey")
...
def Wrapper(packet):
    ...
    def __init__(packet):
        ...
        if types.is_instance(packet, OpenPGP.PublicKeyPacket) or (hasattr(packet, '__getitem__') and types.is_instance(packet[0], OpenPGP.PublicKeyPacket)): # If it's a key (other keys are subclasses of this one)
            self._key = packet
        else:
    ...
    def verifier(h, m, s):
        ...
        if not key or (s.key_algorithm_name() == 'DSA' and not types.is_instance(key, DSAPublicKey)):
            return False
    ...
    def sign(packet, hash='SHA256', keyid=None):
        ...
        if types.is_instance(packet, OpenPGP.SecretKeyPacket) or types.is_instance(packet, RSAPrivateKey) or types.is_instance(packet, DSAPrivateKey) or (hasattr(packet, '__getitem__') and types.is_instance(packet[0], OpenPGP.SecretKeyPacket)):
            key = packet
        ...
        if not (types.is_instance(key, RSAPrivateKey) or types.is_instance(key, DSAPrivateKey)):
        ...
        key_algorithm = None
        if types.is_instance(key, RSAPrivateKey):
            key_algorithm = 'RSA'
        elif types.is_instance(key, DSAPrivateKey):
            key_algorithm = 'DSA'
    ...
    def decrypt(packet):
        ...
        if types.is_instance(packet, OpenPGP.SecretKeyPacket) or types.is_instance(packet, rsa.RSAPrivateKey) or (hasattr(packet, '__getitem__') and types.is_instance(packet[0], OpenPGP.SecretKeyPacket)):
            keys = packet
        else:
            keys = self._key
            self._message = packet
        ...
        if not types.is_instance(keys, rsa.RSAPrivateKey):
            keys = self.__class__(keys)

        for p in self._message:
            if types.is_instance(p, OpenPGP.AsymmetricSessionKeyPacket):
                if types.is_instance(keys, rsa.RSAPrivateKey):
    ...
    def _parse_packet(cls, packet):
        if types.is_instance(packet, OpenPGP.Packet) or types.is_instance(packet, OpenPGP.Message) or types.is_instance(packet, RSAPublicKey) or types.is_instance(packet, RSAPrivateKey) or types.is_instance(packet, DSAPublicKey) or types.is_instance(packet, DSAPrivateKey):
            return packet
    ...
    def convert_key(cls, packet, private=False):
        if types.is_instance(packet, RSAPrivateKey) or types.is_instance(packet, RSAPublicKey) or types.is_instance(packet, DSAPublicKey) or types.is_instance(packet, DSAPrivateKey):
            if (not private) and (types.is_instance(packet, DSAPrivateKey) or types.is_instance(packet, RSAPrivateKey)):
    ...
    def _bytes_to_long(cls, b):
        if hasattr(int, 'from_bytes'):
            ...
            return long(codecs.encode(b, encoding='hex'), 16)
    self._bytes_to_long = _bytes_to_long
    _bytes_to_long = _bytes_to_long
    ...
    def _encode_dsa_der(cls, r, s):
        ...
        return b''.join(der)
    self._encode_dsa_der = _encode_dsa_der
    _encode_dsa_der = _encode_dsa_der
    ...
    def _decode_dsa_der(cls, der):
        ...
        return (r, s)
    self._decode_dsa_der = _decode_dsa_der
    _decode_dsa_der = _decode_dsa_der
    return self
