...
load("@vendor//pyasn1/error", PyAsn1Error="PyAsn1Error")

load("@vendor//rsa", pyrsa="pyrsa")
load("@vendor//rsa", pyrsa_pem="pyrsa_pem")
load("@vendor//rsa", DecryptionError="DecryptionError")

load("@vendor//jose/backends/base", Key="Key")
load("@vendor//jose/backends/_asn1", rsa_private_key_pkcs1_to_pkcs8="rsa_private_key_pkcs1_to_pkcs8", rsa_private_key_pkcs8_to_pkcs1="rsa_private_key_pkcs8_to_pkcs1", rsa_public_key_pkcs1_to_pkcs8="rsa_public_key_pkcs1_to_pkcs8")
load("@vendor//jose/constants", ALGORITHMS="ALGORITHMS")
load("@vendor//jose/exceptions", JWKError="JWKError", JWEError="JWEError")
...
def _legacy_private_key_pkcs8_to_pkcs1(pkcs8_key):
    ...
    if not pkcs8_key.startswith(LEGACY_INVALID_PKCS8_RSA_HEADER + ASN1_SEQUENCE_ID):
        fail(" ValueError(\"Invalid private key encoding\")")
...
def RSAKey(key, algorithm):
    ...
    def is_public():
        return types.is_instance(self._prepared_key, pyrsa.PublicKey)
    self.is_public = is_public

    def public_key():
        if types.is_instance(self._prepared_key, pyrsa.PublicKey):
            return self
    ...
    def to_pem(pem_format='PKCS8'):
        ...
        if types.is_instance(self._prepared_key, pyrsa.PrivateKey):
