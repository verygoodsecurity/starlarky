...
def decrypt(jwe_str, key):
    ...
    if "alg" not in header or "enc" not in header:
        return Error("JWEParseError: alg and enc headers are required!")
...
def _jwe_compact_deserialize(jwe_bytes):
    ...
    encrypted_key = safe(base64url_decode)(encrypted_key_segment)\
        .expect("JWEParseError: Invalid encrypted key")

    iv = safe(base64url_decode)(iv_segment)\
        .expect("JWEParseError: Invalid IV")

    ciphertext = safe(base64url_decode)(cipher_text_segment)\
        .expect("JWEParseError: Invalid cyphertext")
...
#
# # try:
# #     load("@stdlib//collections/abc", Mapping="Mapping")  # Python 3
# # except ImportError:
# #     load("@stdlib//collections", Mapping="Mapping")  # Python 2, will be deprecated in Python 3.8
#
# # load("@vendor//backends", get_random_bytes="get_random_bytes")
# load("@vendor//constants", ALGORITHMS="ALGORITHMS", ZIPS="ZIPS")
# print("----" * 12)
# print(ALGORITHMS)
# # load("@vendor//exceptions", JWEParseError="JWEParseError", JWEError="JWEError")
# # load("@vendor//utils", base64url_decode="base64url_decode", base64url_encode="base64url_encode")
...
#
#
# def _encrypt(plaintext, key, encryption=ALGORITHMS.A256GCM,
#             algorithm=ALGORITHMS.DIR, zip=None, cty=None, kid=None):
#     """Encrypts plaintext and returns a JWE cmpact serialization string.
#
#     Args:
...
#     plaintext = six.ensure_binary(plaintext)  # Make sure it's bytes
#     if algorithm not in ALGORITHMS.SUPPORTED:
#         fail(" JWEError('Algorithm %s not supported.' % algorithm)")
#     if encryption not in ALGORITHMS.SUPPORTED:
#         fail(" JWEError('Algorithm %s not supported.' % encryption)")
#     key = jwk.construct(key, algorithm)
#     encoded_header = _encoded_header(algorithm, encryption, zip, cty, kid)
#
#     plaintext = _compress(zip, plaintext)
...
#         enc = header["enc"]
#         if alg not in ALGORITHMS.SUPPORTED:
#             fail(" JWEError('Algorithm %s not supported.' % alg)")
#         if enc not in ALGORITHMS.SUPPORTED:
#             fail(" JWEError('Algorithm %s not supported.' % enc)")
#
#     except KeyError:
#         fail(" JWEParseError(\"alg and enc headers are required!\")")
#
#     # Verify that the JWE uses a key known to the recipient.
#     key = jwk.construct(key, alg)
#
...
#         # Record whether the CEK could be successfully determined for this
#         # recipient or not.
#         cek_valid = encrypted_key == bytes(r"", encoding='utf-8')
#
#         # When Direct Encryption is employed, let the CEK be the shared
#         # symmetric key.
#         cek_bytes = _get_key_bytes_from_key(key)
...
#             cek_valid = True
#         except NotImplementedError:
#             fail(" JWEError(\"alg {} is not implemented\".format(alg))")
#         except Exception:
#             # Record whether the CEK could be successfully determined for this
#             # recipient or not.
#             cek_valid = False
...
#     except NotImplementedError:
#         fail(" JWEError(\"enc {} is not implemented\".format(enc))")
...
#         auth_tag_check = auth_tag  # GCM check auth on decrypt
#     else:
#         fail(" NotImplementedError(\"enc {} is not implemented!\".format(enc))")
#
#     plaintext = encryption_key.decrypt(cipher_text, iv, aad, auth_tag)
#     if auth_tag != auth_tag_check:
...
#     try:
#         header_segment, encrypted_key_segment, iv_segment, \
#             cipher_text_segment, auth_tag_segment = jwe_bytes.split(bytes([0x2e]), 4)
#         header_data = base64url_decode(header_segment)
#     except ValueError:
#         fail(" JWEParseError('Not enough segments')")
#     except (TypeError, binascii.Error):
#         fail(" JWEParseError('Invalid header')")
#
#     # Verify that the octet sequence resulting from decoding the
#     # encoded JWE Protected Header is a UTF-8-encoded representation
#     # of a completely valid JSON object conforming to RFC 7159
...
#         fail(" JWEParseError('Invalid header string: %s' % e)")
#
#     if not types.is_instance(header, Mapping):
#         fail(" JWEParseError('Invalid header string: must be a json object')")
#
#     try:
#         encrypted_key = base64url_decode(encrypted_key_segment)
#     except (TypeError, binascii.Error):
#         fail(" JWEParseError('Invalid encrypted key')")
#
#     try:
#         iv = base64url_decode(iv_segment)
#     except (TypeError, binascii.Error):
#         fail(" JWEParseError('Invalid IV')")
#
#     try:
#         ciphertext = base64url_decode(cipher_text_segment)
#     except (TypeError, binascii.Error):
#         fail(" JWEParseError('Invalid cyphertext')")
#
#     try:
#         auth_tag = base64url_decode(auth_tag_segment)
#     except (TypeError, binascii.Error):
#         fail(" JWEParseError('Invalid auth tag')")
#
#     return header, header_segment, encrypted_key, iv, ciphertext, auth_tag
#
#
...
#     """
#     if zip not in ZIPS.SUPPORTED:
#         fail(" NotImplementedError(\"ZIP {} is not supported!\")")
#     if zip == None:
#         compressed = plaintext
#     elif zip == ZIPS.DEF:
#         compressed = zlib.compress(plaintext)
#     else:
#         fail(" NotImplementedError(\"ZIP {} is not implemented!\")")
#     return compressed
#
#
# def _decompress(zip, compressed):
...
#         # Get the last half of an octal key as the cek
#         cek_bytes = _get_key_bytes_from_key(key)
#         wrapped_cek = bytes(r"", encoding='utf-8')
#     else:
...
#         num_bits = 512
#     else:
#         fail(" NotImplementedError(\"{} not supported\".format(enc))")
#     cek_bytes = get_random_bytes(num_bits // 8)
#     return cek_bytes
#
#
...
#     encoded_cipher_text = base64url_encode(cipher_text)
#     encoded_auth_tag = base64url_encode(auth_tag)
#     return encoded_header + bytes([0x2e]) + encoded_encrypted_cek + bytes([0x2e]) + \
#         encoded_iv + bytes([0x2e]) + encoded_cipher_text + bytes([0x2e]) + encoded_auth_tag
#
