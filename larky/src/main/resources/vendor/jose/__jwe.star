...
load("@vendor//Crypto/Random/Random", urandom="urandom")
...
def _decrypt_and_auth(cek_bytes, enc, cipher_text, iv, aad, auth_tag):
    ...
    if enc in ["A256GCM"]:
        ...
        fail("enc {} is not implemented!".format(enc))


    # plaintext = encryption_key.decrypt(cipher_text, iv, aad, auth_tag)
    plaintext = encryption_key.decrypt(cipher_text)
    print("===========")
    print(plaintext)
    print("===========")

    if auth_tag != auth_tag_check:
        fail("Invalid JWE Auth Tag")
...
def _jwe_compact_deserialize(jwe_bytes):
    ...
    # header_segment, encrypted_key_segment, iv_segment, cipher_text_segment, auth_tag_segment = jwe_bytes.split(bytes('.'), 4)

    # workaround
    jwe_str = str(jwe_bytes).split('.', 4)
    header_segment, encrypted_key_segment, iv_segment, cipher_text_segment, auth_tag_segment = \
        bytes(jwe_str[0], encoding='utf-8'), bytes(jwe_str[1], encoding='utf-8'), \
        bytes(jwe_str[2], encoding='utf-8'), bytes(jwe_str[3], encoding='utf-8'), \
        bytes(jwe_str[4], encoding='utf-8')
    # end workaround
    ...
    header = json.loads(str(header_data))

    if not types.is_dict(header):
        fail('Invalid header string: must be a json object')
...
def _compress(zip, plaintext):
    ...
    """
    if zip not in []:
        fail("ZIP {} is not supported!".format(zip))
    if zip == None:
        compressed = plaintext
    else:
        fail("ZIP {} is not implemented!".format(zip))
    return compressed
...
def _get_direct_key_wrap_cek(key):
    ...
    if jwk_data["kty"] == "oct":
        ...
        fail("JWK type {} not supported!".format(jwk_data['kty']))
...
def _jwe_compact_serialize(encoded_header, encrypted_cek, iv, cipher_text, auth_tag):
    ...
    return encoded_header + bytes(".", encoding='utf-8') + encoded_encrypted_cek + bytes(".", encoding='utf-8') + \
           encoded_iv + bytes(".", encoding='utf-8') + encoded_cipher_text + bytes(".", encoding='utf-8') + encoded_auth_tag


def encrypt(plaintext, key, encryption="A256GCM",
            algorithm="DIR", zip=None, cty=None, kid=None):
    ...
    if algorithm not in ["DIR"]:
        fail('Algorithm {} not supported.'.format(algorithm))
    if encryption not in ["A256GCM"]:
        fail('Algorithm {} not supported.'.format(encryption))
...
def decrypt(jwe_str, key):
    ...
    if alg not in ["DIR", "A256GCMKW"]:
        fail('Algorithm {} not supported.'.format(alg))
    if enc not in ["A256GCM"]:
        fail('Algorithm {} not supported.'.format(enc))
    ...
    #     try:
    #     cek_bytes = key.decrypt(encrypted_key)
    #
    #         # Record whether the CEK could be successfully determined for this
    #         # recipient or not.
        cek_valid = True
...
def _get_random_cek_bytes_for_enc(enc):
    ...
    #     num_bits = 512
        ...
        fail("{} not supported", enc)
    cek_bytes = urandom(num_bits // 8)
    return cek_bytes
