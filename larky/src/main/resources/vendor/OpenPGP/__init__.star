...
def S2K(salt=b"BADSALT", hash_algorithm=10, count=65536, type=3):
    self = larky.mutablestruct(__name__='S2K', __class__=S2K)
    ...
    self.decode_s2k_count = decode_s2k_count
    ...
    def encode_s2k_count(cls, iterations):
        ...
        return result
    self.encode_s2k_count = encode_s2k_count
...
def Message(packets=None):
    ...
    def parse(input_data):
        ...
        return m
    classmethods["parse"] = parse

    self = larky.mutablestruct(__name__='Message', __class__=Message, **classmethods)
...
def _Packet():
    ...
    def parse(input_data):
        ...
        if tag:
            rval = Result.Ok(Packet.tags).map(operator.itemgetter(tag))
            if rval.is_ok:
            ...
            if not packet_read.is_ok:
                fail("OpenPGPException: Not enough bytes")
            # Remove excess bytes
            packet_read = Result.Ok(packet.read_bytes).map(lambda x: x.read(packet.length))
            if not packet_read.is_ok:
                fail("OpenPGPException: Not enough bytes")
    ...
    cls.tags = {}  # Actual data at end of file
...
def SignaturePacket(data, key_algorithm, hash_algorithm):
    ...
    def __init__(data, key_algorithm, hash_algorithm):
        ...
        return self
    cls.__call__ = __init__
    self = cls(data, key_algorithm, hash_algorithm)
    ...
    self.subpacket_types = SUBPACKET_TYPES
    return self
...
# SignaturePacket.EmbeddedSignaturePacket = SignaturePacket.subpacket_types[
#     32
# ]
...
def PublicKeyPacket(keydata=None, version=4, algorithm=1, timestamp=time()):
    ...
    def __init__(keydata, version, algorithm, timestamp):
        ...
        return self
    cls.__call__ = __init__
    self = cls(keydata, version, algorithm, timestamp)
...
def SecretKeyPacket(keydata=None, version=4, algorithm=1, timestamp=time()):
    ...
    _super_read = self.read
    def read():
        _super_read()  # All the fields from PublicKey
    ...
    _parent_body = self.body
    def body():
        b = _parent_body() + pack("!B", self.s2k_useage)
        secret_material = b""
        if self.s2k_useage == 255 or self.s2k_useage == 254:
...
#     self.__getitem__ = __getitem__
#     return self
# def EncryptedDataPacket(keydata=None, version=4, algorithm=1, timestamp=time()):
...
#     return self
# def MarkerPacket(keydata=None, version=4, algorithm=1, timestamp=time()):
...
def LiteralDataPacket(data=None, format="b", filename="data", timestamp=time()):
    ...
    cls.__class__ = LiteralDataPacket
    ...
    def __init__(data, format, filename, timestamp):
        ...
        return self
    cls.__call__ = __init__
    self = cls(data, format, filename, timestamp)
    ...
    self.body = body
    return self
# def TrustPacket(data=None, format="b", filename="data", timestamp=time()):
...
def UserIDPacket(name="", comment=None, email=None):
    ...
    self.body = body
    return self
# def UserAttributePacket(name="", comment=None, email=None):
...
#     """
#     self = larky.mutablestruct(__name__='IntegrityProtectedDataPacket', __class__=IntegrityProtectedDataPacket)
#
#     def __init__(data, version):
