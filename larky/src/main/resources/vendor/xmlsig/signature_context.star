...
def SignatureContext():
    ...
    def sign(node):
        ...
        :return: None
        """
        signed_info = node.find("ds:SignedInfo", namespaces=ns.NS_MAP)
        signature_method = signed_info.find(
            "ds:SignatureMethod", namespaces=ns.NS_MAP
        ).get("Algorithm")
        key_info = node.find("ds:KeyInfo", namespaces=ns.NS_MAP)
        if key_info != None:
    ...
    def fill_key_info(key_info, signature_method):
        ...
        :return: None
        """
        x509_data = key_info.find("ds:X509Data", namespaces=ns.NS_MAP)
        if x509_data != None:
            self.fill_x509_data(x509_data)
        key_name = key_info.find("ds:KeyName", namespaces=ns.NS_MAP)
        if key_name != None and self.key_name != None:
            key_name.text = self.key_name
        key_value = key_info.find("ds:KeyValue", namespaces=ns.NS_MAP)
        if key_value != None:
            key_value.text = "\n"
    ...
    def fill_x509_data(x509_data):
        ...
        x509_certificate = x509_data.find(
            "ds:X509Certificate", namespaces=ns.NS_MAP
        )
        if x509_certificate != None:
            ...
            for certificate in self.ca_certificates:
                certificate_node = create_node(
                    "X509Certificate", None, ns.DSigNs, tail="\n"
                )
    ...
    def fill_x509_issuer_name(x509_issuer_serial):
        ...
        x509_issuer_name = x509_issuer_serial.find(
            "ds:X509IssuerName", namespaces=ns.NS_MAP
        )
        ...
        x509_issuer_number = x509_issuer_serial.find(
            "ds:X509SerialNumber", namespaces=ns.NS_MAP
        )
    ...
    def fill_signed_info(signed_info):
        ...
        for reference in signed_info.findall(
            "ds:Reference", namespaces=ns.NS_MAP
        ):
    ...
    def transform(transform, node):
        ...
        if method.href not in constants.TransformUsageDSigTransform:
    ...
    def calculate_reference(reference, sign=True):
        ...
        transforms = reference.find("ds:Transforms", namespaces=ns.NS_MAP)
        if transforms != None:
        ...
        digest_value = self.digest(
            reference.find("ds:DigestMethod", namespaces=ns.NS_MAP).get(
                "Algorithm"
            ),
            node,
        )
        if not sign:
            return (
                ...
                == reference.find("ds:DigestValue", namespaces=ns.NS_MAP).text
            )

        reference.find(
            "ds:DigestValue", namespaces=ns.NS_MAP
        ).text = digest_value
    ...
    def calculate_signature(node, sign=True):
        ...
        :return: None
        """
        signed_info_xml = node.find("ds:SignedInfo", namespaces=ns.NS_MAP)
        canonicalization_method = signed_info_xml.find(
            "ds:CanonicalizationMethod", namespaces=ns.NS_MAP
        ).get("Algorithm")
        ...
        if not sign:
            signature_value = node.find(
                "ds:SignatureValue", namespaces=ns.NS_MAP
            ).text
