...
def Request(
    ...
    **kwargs,
    ...
    # LarkySanitizedHttpMessage backwards compatibility
    def body(charset="utf-8"):
        _body = _get_data()
        if types.is_bytelike(_body):
            return _body.decode(charset)
        return _body
    self.body = body

    # LarkySanitizedHttpMessage backwards compatibility
    def set_body(body, charset="utf-8"):
        if not types.is_bytelike(body):
            self.data = body.decode(encoding=charset)
        else:
            self.data = body
        return self
    ...
    def raw_body():
        _body = _get_data()
        if not types.is_bytelike(_body):
            return codecs.encode(_body, encoding="utf-8")
        return _body
    ...
    # non standard extension (for serialization..)
    self.url = larky.property(_get_full_url)
    # LarkySanitizedHttpMessage backwards compatibility
    self.uri = _get_full_url
    self.set_uri = _set_full_url

    def _get_headers():
        # a really awkward dict shim for backward compatibility
        # with LarkySanitizedHttpMessage and python compatibility as well.
        return larky.struct(
            keys = self._headers.keys,
            values = self._headers.values,
            items = self._headers.items,
            setdefault = self._headers.setdefault,
            update = self._headers.update,
            pop = self._headers.pop,
            popitem = self._headers.pop,
            get = self._headers.get,
            __getitem__ = lambda key: operator.getitem(self._headers, key),
            # TODO(mahmoudimus): overload []=
            #  only __setitem__ that does not get an operator overload..
            __setitem__ = self.add_header,
            __delitem__ = lambda key: operator.delitem(self._headers, key),
            __contains__ = lambda needle: operator.contains(self._headers, needle),
            __iter__ = lambda: iter(self._headers),
            __call__ = lambda: self._headers,
        )
    ...
    # full_url = full_url.deleter(full_url)

    # def data():
    #     self.data = None
    # self.data = data
    # data = data.deleter(data)
    ...
    def add_header(key, val):
        ...
        if self.kwargs.get('capitalize_keys', False):
            key = key.capitalize()
    ...
    self = __init__(url, data, headers, origin_req_host, unverifiable, method, **kwargs)
    return self
