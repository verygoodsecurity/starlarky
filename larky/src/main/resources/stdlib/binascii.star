...
def b2a(bString, separator=' '):
    ...
    return separator.join(['%.2x' % ord(c) for c in bString])
    ...
    for i in range(len(ascii_byte_list)):
        if i != 0:
            if i % 16 == 0:  # add trailer + \n every 16 bytes
                ...
                pretty_list += '  '  # add an extra space every 8 bytes
            else:
                pretty_list += ' '  # space between bytes
        else:
            pretty_list += '\n' + frnt
        pretty_list += ascii_byte_list[i]
        if string.find(viewable, chr(int(ascii_byte_list[i],
                                         16))) >= 0:  # if printable and not white space
...
def b2a_pter(s, frnt='        ', gap='   ', text_trailer=None):
    ...
    if len(s) > 50: return b2a_pt(s, '        ', gap=gap,
                                  text_trailer=text_trailer)
    for char in s:
